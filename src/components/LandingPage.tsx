...
const DL_WINDOWS =
  `${GITHUB_RELEASE}/download/AI._0.1.5_x64-setup.exe`;
...
// ---------------------------------------------------------------------------
// Counter hook — animate a number from 0 to `end` when visible.
// ---------------------------------------------------------------------------
...
export function LandingPage({ onSignIn }: Props) {
  ...
  // Mouse-following glow orb on hero section.
  useEffect(() => {
  ...
  // Parallax: move hero screenshot on scroll.
  useEffect(() => {
    const onScroll = () => {
      const y = window.scrollY;
      const el = document.querySelector('.lp-hero__screenshot') as HTMLElement | null;
  ...
  return (
    <div className="lp">
      ...
      <nav className={`lp-nav ${navSolid ? 'lp-nav--solid' : ''}`}>
        <div className="lp-nav__inner">
          ...
          <div className="lp-nav__links">
            <button type="button" onClick={() => scrollTo(featuresRef)} className="lp-nav__link">
              機能
            </button>
            <button type="button" onClick={() => scrollTo(downloadRef)} className="lp-nav__link">
              ダウンロード
            </button>
            <button type="button" onClick={handleSignIn} className="lp-nav__cta">
              Web版を使う
              <ArrowRight size={14} />
            </button>
          </div>
        ...
        <div className="lp-hero__content">
          ...
          <div className="lp-hero__actions lp-anim-word" style={{ animationDelay: '0.7s' }}>
            <button type="button" onClick={handleSignIn} className="lp-btn lp-btn--primary">
              Web版を無料で使う
              <ArrowRight size={16} className="lp-btn__arrow" />
            </button>
          ...
          <div className="lp-hero__screenshot lp-anim-word" style={{ animationDelay: '0.9s' }}>
            <div className="lp-hero__screenshot-placeholder">
              <Monitor size={48} className="text-slate-400" />
              <span className="text-sm text-slate-500">アプリのスクリーンショット</span>
            </div>
          </div>
          <button
            type="button"
      ...
      <section ref={featuresRef} className="lp-features">
        <div className="lp-section-inner">
          <h2 className="lp-section-title lp-fade-in">
            就活に必要な機能を、すべてひとつに
          </h2>
          ...
          <div className="lp-features__grid">
            <FeatureCard icon={<Kanban size={24} />} title="就活スプレッドシート" description="表・Kanban・カレンダーの 3 ビューで選考状況を一覧管理。ES 締切・面接日程も一目瞭然。" delay={0} />
            <FeatureCard icon={<FileText size={24} />} title="WYSIWYG エディタ" description="Markdown ベースのリッチエディタで企業分析・ES 下書き・面接メモを作成。色・装飾・見出しも自由自在。" delay={1} />
            <FeatureCard icon={<Sparkles size={24} />} title="AI 企業分析" description="Gemini + Google 検索で企業情報を自動生成。オンボーディング時に選択した企業の資料が即座に揃います。" delay={2} />
            <FeatureCard icon={<Shield size={24} />} title="ローカルファースト" description="データはあなたの PC に保存。クラウド同期は任意の暗号化バックアップとして使えます。" delay={3} />
            <FeatureCard icon={<CalendarDays size={24} />} title="カレンダー & Kanban" description="ES 提出・面接・Web テストのスケジュールをカレンダーで俯瞰。Kanban でステータス管理も。" delay={4} />
            <FeatureCard icon={<Monitor size={24} />} title="マルチフォーマット対応" description="Markdown だけでなく、PDF・画像・Word・Excel もアプリ内で閲覧。企業フォルダを丸ごと管理。" delay={5} />
          </div>
        </div>
      </section>
      ...
      <section ref={downloadRef} className="lp-download">
        <div className="lp-section-inner">
          ...
          <div className="lp-download__cards lp-fade-in">
            <div className="lp-download__card">
              ...
              <a href={DL_MACOS} className="lp-btn lp-btn--outline">
            ...
            <div className="lp-download__card">
              ...
              <a href={DL_WINDOWS} className="lp-btn lp-btn--outline">
                <Download size={16} />ダウンロード (.exe)
              </a>
            </div>
            <div className="lp-download__card lp-download__card--highlight">
              ...
              <button type="button" onClick={handleSignIn} className="lp-btn lp-btn--primary lp-btn--sm">
      ...
      <section className="lp-cta">
        <div className="lp-section-inner">
          ...
          <button type="button" onClick={handleSignIn} className="lp-btn lp-btn--primary lp-btn--lg lp-fade-in">
